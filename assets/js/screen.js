/* ============================================================
   ...
   Pointer-events are off on the whole layer so it can never steal a
   click from pointer-lock; sound is toggled with a key (a real user
   gesture) so it gets past autoplay policy.
   ============================================================ */
...
function build() {
  ...
  el.style.pointerEvents = "none"; // never steal a click from pointer-lock
...
// build the embed URL for a parsed stream at a given mute state
function srcFor(s, m) {
  if (!s) return "";
  if (s.platform === "twitch") {
    const host = location.hostname || "localhost";
    // twitch demands the embedding host as a parent; localhost stays for dev
...
export const screen = {
  ...
  // put a stream up (or re-show one). idempotent: same stream + same mute =
  // no reload, so the 5 s re-announce never restarts playback.
  show(s, m) {
    if (!s) return;
    build();
    const wantMute = m !== undefined ? m : muted;
    const same = cur && cur.platform === s.platform && cur.kind === s.kind && cur.id === s.id;
    if (mounted && same && wantMute === muted) return;
    cur = { platform: s.platform, kind: s.kind, id: s.id };
    muted = wantMute;
    iframe.src = srcFor(cur, muted);
    frame.style.display = "";
    mounted = true;
  },
  ...
  current: () => cur,
  isMuted: () => muted,
  setMuted(m) { muted = !!m; if (mounted && cur) iframe.src = srcFor(cur, muted); },
