...
- **A person must be legible in an unlit corner.** ALL room light here comes from the window, so a `MeshLambertMaterial` body standing anywhere the window can't reach renders pure BLACK. The face and the shirt logo are `MeshBasicMaterial` and light themselves, so a peer becomes a silhouette with floating eyes — which is exactly what it looked like. Avatar materials carry an `emissive` floor (`avatar-builder.js`, `floorOf`). Two parts, and the second is the one that matters: a SHARE of the colour keeps it that person's colours, and a small ABSOLUTE lift keeps dark clothing readable, because 30% of near-black is still near-black. Emissive costs no light budget, can't leak through a wall, and survives the toon pass.
