...
**The wall fills up, and a full wall leaves NO server-side trace.** `canPlace` runs in the browser and refuses before any request is made, so "notes stopped working" shows up as an *absence* of POSTs in the Supabase logs, not as an error. All three bedroom walls were full from 2026-08-18 (the last note that landed) until it was caught on 2026-08-22, and the only reason anyone found out was a visitor mentioning it. If notes ever stop landing, measure `notesWall.canPlace()` against the live data (drive the LIVE site headlessly — `config.js` ships the anon key, so a local page talks to the real DB unless you stub `METRO_CONFIG`) before you go looking for a bug. The room is smaller than it looks: the thirteen acoustic slabs push no-post rects that leave only 15–23% of each wall postable before a single note goes up, so capacity is set by `NOTE_W`/`PAD`/`BAND_HI` in notes3d.js and it is not large.
